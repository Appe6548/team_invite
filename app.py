...
def jwt_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not auth_header.startswith('Bearer '):
            return jsonify({'error': '未初始化身份，请刷新页面'}), 401
        token = auth_header[7:]
        payload = verify_jwt_token(token)
        if not payload:
            return jsonify({'error': '身份已过期，请刷新页面'}), 401
...
def auth_anonymous():
    """创建匿名用户并返回 JWT（无需 OAuth）"""
    username = f"guest_{secrets.token_hex(4)}"
    conn = get_db()
    cursor = conn.execute(
        'INSERT INTO users (username, name, avatar_template, trust_level) VALUES (?, ?, ?, ?)',
        (username, '', '', 0),
    )
